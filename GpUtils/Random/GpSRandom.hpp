...
public:
    ...
    virtual GpByteArray ByteArray       (size_t aSize) override final;
    virtual void        ByteArray       (GpSpanByteRW   aSpanByteRW) override final;
    virtual void        ByteArray       (GpByteWriter&  aDataWriter,
                                         size_t         aSize) override final;

    template<typename T>
    T&                  ByteArray       (T&     aBufferToWrite,
                                         size_t aSize);
...
template<typename T>
T&  GpSRandom::ByteArray
(
    T&              aBufferToWrite,
    const size_t    aSize
)
{
    return GpRandomIf::ByteArray<T>(aBufferToWrite, aSize);
}
